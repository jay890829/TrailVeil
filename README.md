...
In Google Cloud, restrict the key to Android apps with package name `app.trailveil` and the SHA-1 of the certificate that signs your build - for the `googlePoc` build type that is your own Android debug certificate, which is also why such a build cannot replace an installed release APK in place. The key is a build input by construction: the Maps SDK resolves it from a compiled resource, so there is no runtime entry and nothing in the app ever asks for it.
