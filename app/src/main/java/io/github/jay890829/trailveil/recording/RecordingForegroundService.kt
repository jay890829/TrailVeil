...
import io.github.jay890829.trailveil.data.location.LocationPermissionException
import io.github.jay890829.trailveil.data.location.LocationProviderDisabledException
import io.github.jay890829.trailveil.data.location.LocationProviderUnavailableException
...
class RecordingForegroundService : Service() {
    ...
    private suspend fun handleStickyRestart(command: Command.StickyRestart) {
        ...
        try {
            when (dependencies.recordingRepository.recover(
                ...
                io.github.jay890829.trailveil.data.recording.RecoveryDisposition.ACTIVE_ROTATED -> {
                    val sessionId = requireNotNull(dependencies.recordingRepository.state().sessionId)
                    notifier.show(this, sessionId)
                    startCollector(sessionId)
                }
                io.github.jay890829.trailveil.data.recording.RecoveryDisposition.ACTIVE_ALREADY_RECOVERED -> {
                    // The repository did not grant this service a location-owner token.
                    stopRuntime(command.startId)
                }
                io.github.jay890829.trailveil.data.recording.RecoveryDisposition.STARTING_FAILED,
                io.github.jay890829.trailveil.data.recording.RecoveryDisposition.NOTHING_TO_RECOVER ->
                    stopRuntime(command.startId)
            }
        } catch (cancelled: CancellationException) {
            throw cancelled
    ...
    companion object {
        const val ACTION_START = "io.github.jay890829.trailveil.action.START_RECORDING"
        const val ACTION_STOP = "io.github.jay890829.trailveil.action.STOP_RECORDING"
        const val EXTRA_SESSION_ID = "io.github.jay890829.trailveil.extra.SESSION_ID"
