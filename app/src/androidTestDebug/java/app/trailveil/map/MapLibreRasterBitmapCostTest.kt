...
/** Same test APK on accepted b58 and X2. Both pack on worker; only X2 creates Bitmap there. */
class MapLibreRasterBitmapCostTest {
    @get:Rule val composeRule = createComposeRule()

    @Test fun workerBitmapAndSdkSubmissionCosts() {
        val args = InstrumentationRegistry.getArguments()
        assumeTrue(args.getString("x2Cost") == "true")
        ...
        check(appHash == args.getString("x2ExpectedApkHash"))
        val preparedMode = true
        val workerBitmapMode = args.getString("x2WorkerBitmap") == "true"
        val ownerClass = if (workerBitmapMode) Class.forName("app.trailveil.map.PreparedRasterFogBitmap") else null
        val ownerPrepare = ownerClass?.methods?.single { it.name == "prepare" && it.parameterCount == 2 }
        val ownerBitmap = ownerClass?.getMethod("bitmapFor", FogTileMosaic::class.java)
        val ownerClose = ownerClass?.getMethod("close")
        val sizes = (args.getString("x2SizeOrder") ?: "256,768,1280").split(',').map(String::toInt)
        ...
        composeRule.setContent {
            TrailVeilMapSurface(modifier = Modifier.fillMaxSize(),
                provider = MapProviderConfiguration("x2-cost", "https://tiles.invalid/x2-cost"),
        ...
        instrumentation.runOnMainSync {
            ...
            style.addSource(GeoJsonSource("x2-anchor-source", FeatureCollection.fromFeatures(emptyList())))
            style.addLayer(LineLayer("x2-anchor", "x2-anchor-source"))
        }
        ...
        for (size in sizes) {
            ...
            repeat(7) { sample ->
                ...
                val sourceId = "x2-$size-$sample-source"
                val layerId = "x2-$size-$sample-layer"
                val preparedRef = AtomicReference<Any?>()
                val owner = ownerClass?.getConstructor()?.newInstance()
                ...
                val closeCpu = AtomicLong()
                val closeWall = AtomicLong()
                ...
                if (preparedMode) {
                    ...
                    preparedRef.set(runBlocking(Dispatchers.Default) {
                        ...
                        val prepared: Any = suspendCoroutineUninterceptedOrReturn { continuation ->
                            try { if (workerBitmapMode) checkNotNull(ownerPrepare).invoke(owner, mosaic, continuation) else checkNotNull(prepareMethod).invoke(null, mosaic, continuation) }
                ...
                instrumentation.runOnMainSync {
                    ...
                    val bitmap = if (preparedMode) {
                        try { (if (workerBitmapMode) checkNotNull(ownerBitmap).invoke(owner, mosaic) else checkNotNull(bitmapMethod).invoke(preparedRef.get(), mosaic)) as Bitmap }
                        catch (failure: java.lang.reflect.InvocationTargetException) { throw failure.targetException }
                    ...
                    style.addLayerBelow(RasterLayer(layerId, sourceId).withProperties(
                        ...
                        PropertyFactory.rasterOpacity(1f)), "x2-anchor")
                ...
                val actualRaw = if (workerBitmapMode) legacyPixels(mosaic.mask) else if (preparedMode) checkNotNull(pixelsField).get(preparedRef.get()) as IntArray else checkNotNull(rawRef.get())
                ...
                // Production releases on Main after installation progress, while SDK sources exist.
                // The pixel oracle above is outside every timed interval. Baseline has no explicit close.
                if (workerBitmapMode) instrumentation.runOnMainSync {
                    val cpu = Debug.threadCpuTimeNanos()
                    val started = System.nanoTime()
                    checkNotNull(ownerClose).invoke(owner)
                    closeWall.set(System.nanoTime() - started)
                    closeCpu.set(Debug.threadCpuTimeNanos() - cpu)
                    assertTrue(checkNotNull(bitmapRef.get()).isRecycled)
                    assertNotNull(style.getSource(sourceId))
                }
                samples.put(JSONObject().put("sample", sample).put("packingCpuNs", packingCpu.get())
                    ...
                    .put("closeCpuNs", closeCpu.get()).put("closeWallNs", closeWall.get()).put("sourceConstructorNs", sourceWall.get()).put("styleMutationNs", mutationWall.get())
                ...
                if (!workerBitmapMode) bitmapRef.get()?.recycle()
                assertTrue(checkNotNull(bitmapRef.get()).isRecycled)
            ...
            val record = JSONObject().put("size", size).put("pixels", size * size).put("prepared", preparedMode)
                ...
                .put("workerBitmap", workerBitmapMode).put("sizeOrder", sizes.joinToString(",")).put("rawArgbSha256", rawHash).put("bitmapSha256", bitmapHash).put("samples", samples)
            instrumentation.sendStatus(0, Bundle().apply { putString("stream", "MAPLIBRE_BITMAP_COST $record\n") })
        }
    }
