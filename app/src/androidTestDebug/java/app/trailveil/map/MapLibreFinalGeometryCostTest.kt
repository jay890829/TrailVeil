...
/** Actual UI cost with output hashing only after the installed endpoint; not a frame/cover benchmark. */
@RunWith(AndroidJUnit4::class)
class MapLibreFinalGeometryCostTest {
    @get:Rule val composeRule = createAndroidComposeRule<ComponentActivity>()
    @Test fun actualNativeReturnAndMissCosts() {
        ...
        try {
            ...
            for (kind in orders) {
                ...
                for ((index, offset) in offsets.withIndex()) {
                    ...
                    val geometry = (endpoint.render.presentation as FogNativePresentation).geometry
                    // No deep observer or output hash runs before this completed installed endpoint.
                    val outputHash = geometryHash(geometry); hashes += outputHash
                    val row = JSONObject().put("case", "maplibre-z$zoom-5x5-$kind").put("step", index + 1)
                        ...
                        .put("outputSha256", outputHash).put("diagnostics", geometry.diagnostics)
                    ...
                    rows.put(row)
                    instrumentation.sendStatus(0, Bundle().apply { putString("stream", "X4_UI_ROW $row\n") })
                }
                if (kind == "return") { check(hashes[0] == hashes[4]); check(hashes[1] == hashes[3]) }
            }
            instrumentation.sendStatus(0, Bundle().apply { putString("stream", "X4_UI_DONE ${JSONObject()
    ...
    private fun geometryHash(geometry: FogNativeGeometry): String = hash(buildString {
        ...
        geometry.polygons.forEach { polygon ->
            ...
            (listOf(polygon.shell) + polygon.holes).forEach { ring ->
                ...
                ring.forEach { append(it.latitude.toRawBits()).append(';').append(it.longitude.toRawBits()).append(';') }
            }
        }
    }.toByteArray())
