 ...
 */
internal class TrackRegionGeometryEngine : FogNativeGeometryEngine {
    ...
    override fun rawReadWindows(bounds: FogTileBounds, style: FogRenderStyle): List<ViewportBounds>? {
        if (!supportsBounds(bounds)) return null
        val plan = partitionPlan(bounds) ?: return null
        val keys = buildList {
            for (y in plan.y0..plan.y1) for (x in plan.x0..plan.x1) add(FogTileKey(PARTITION_ZOOM, x, y, FogRenderVersions.CURRENT))
        }
        // containsKey does not promote access-order LRU entries. A later invalidation merely makes
        // this warm hint a safe false negative. Any miss includes ALL domains, since building a
        // preceding miss can evict an entry that was warm at preview time.
        if (radius == style.revealRadiusMeters && keys.all { cache.containsKey(it) }) return emptyList()
        return keys.map { partitionBounds(it.x, it.y) }
    }

    private data class PartitionPlan(
        val west: Double, val east: Double, val north: Double, val south: Double,
        val x0: Int, val x1: Int, val y0: Int, val y1: Int,
    )

    private fun supportsBounds(bounds: FogTileBounds): Boolean = !(bounds.westLongitude < -180.0 || bounds.eastLongitude > 180.0 ||
        bounds.eastLongitude <= bounds.westLongitude || bounds.southLatitude < -80.0 || bounds.northLatitude > 80.0)

    private fun partitionPlan(bounds: FogTileBounds): PartitionPlan? {
        ...
        if ((x1 - x0 + 1).toLong() * (y1 - y0 + 1) > MAX_REQUEST_PARTITIONS) return null
        return PartitionPlan(west, east, north, south, x0, x1, y0, y1)
    }
    ...
    private suspend fun renderPrepared(
        ...
        if (!supportsBounds(bounds)) return fallback("wrapped-or-polar")
        val (west, east, north, south, x0, x1, y0, y1) = partitionPlan(bounds) ?: return fallback("partition-budget")
    ...
    private suspend fun buildPartition(
        ...
        suspend fun bufferRun(points: List<Coordinate>, minimumRadius: Double) {
            ...
            val area = clipTrackAreaToRectangle(BufferOp.bufferOp(line, minimumRadius, parameters), clip)
        ...
        return if (buffers.isEmpty()) factory.createPolygon() else UnaryUnionOp.union(buffers)
    }
